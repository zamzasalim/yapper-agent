...
flowchart TD
    ...
    A3 --> A4{User in DB?}
    A4 -->|New|      A5[POST /api/user · Create record\nScrapeBadger: followers + blue]
    A4 -->|Existing| A6[POST /api/user · Sync wallet\nRefresh followers + blue]
    ...
    CL1 --> CL2{Job Type}
    CL2 -->|Repost $0.50\nLike & Reply $0.20|    CL3[Fixed price]
    CL2 -->|Content / Campaign\ntier pricing|     CL4[Select tier\nNano · Micro · Mid · Macro\nSet creator count]
    CL2 -->|Custom\nadmin approval required|      CL5[Free-form brief\nNo payment yet]
    ...
    CR4 -->|> 1  Campaign| CR6[job_completions row\nslots_taken++]
    ...
    %% ══════════════════════════════════════════════════════════════
    %%  AUTO-EXPIRE  (cron + manual trigger)
    %% ══════════════════════════════════════════════════════════════
    ...
    subgraph DB_FIELDS["Key DB Fields on jobs"]
        direction LR
        DB1["completed_at:     timestamptz — set when status → completed"]
        DB2["cancel_reason:    text        — set when status → cancelled"]
        DB3["deadline_override: timestamptz — admin override; replaces created_at + deadline_hours in expiry check"]
    end

    %% ══════════════════════════════════════════════════════════════
    %%  NAVIGATION LINKS (dotted)
    %% ══════════════════════════════════════════════════════════════
    A7 -.->|Connect flow| TG1
    ...
    class AD_PEND,AD1D,AD2,AD3,AD4,AD5,AD6,AD7,AD_ACT,ADMHIDE,ADMEXT,ADMCANCEL,AD_CAN,CANVIEW,RESTORE,CANDEL,CANREASON,CANEXP,CANADM,CANCLI admin
